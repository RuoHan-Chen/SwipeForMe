...
jest.mock('../../../pages/Dashboard/ActivityPanel/ActivityPanel', () => {
  const original = jest.requireActual('../../../pages/Dashboard/ActivityPanel/ActivityPanel');
  return {
    ...original,
...
describe('SellerView Component', () => {
  beforeEach(() => {
    (getCurrentUserAvailability as jest.Mock).mockResolvedValue(mockAvailabilities);
    (getCurrentUserTransactionsAsSeller as jest.Mock).mockResolvedValue(mockTransactions);
    jest.clearAllMocks();
  });
  ...
  test('displays no availabilities message when empty', async () => {
    ...
    expect(await screen.findByText(/You don't have any upcoming availabilities/i)).toBeInTheDocument();
  });
  ...
  test('handles error state', async () => {
    ...
    (getCurrentUserAvailability as jest.Mock).mockRejectedValue(new Error('Failed to fetch'));
    (getCurrentUserTransactionsAsSeller as jest.Mock).mockResolvedValue([]);
